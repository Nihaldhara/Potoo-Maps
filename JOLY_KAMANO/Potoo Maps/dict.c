...
void *Dict_Insert(Dict *dict, char *key, void *value)
{
    ...
    {
        // La donn�e est d�j� pr�sente dans l'arbre
    ...
    // Equilibre l'arbre � partir du parent du nouveau noeud
...
void Dict_Balance(Dict *dict, DictNode *node)
{
    // Remonte �ventuellement jusqu'� la racine pour r��quilibrer l'arbre
    while (node != NULL)
    {
...
void *Dict_Remove(Dict *dict, char *key)
{
    ...
    else
    {
        ...
        // On l'�change avec sa valeur imm�diatement inf�rieure (qui n'a plus de fils droit)
        DictNode *maxLeft = DictNode_Maximum(node->leftChild);
        KVPair pair = node->pair;
        node->pair = maxLeft->pair;
        maxLeft->pair = pair;

        // Puis on le supprime comme pr�c�demment
    ...
    // Equilibre l'arbre � partir du parent du noeud supprim�
...
/*Dict* createAVLDictionary(char** words, int wordcount)
{
    ...
    //On met toutes les mots dans le dictionnaire pr�c�dement cr�e
    //La cl� �tant le mot et la valeur le nombre de fois que la cl� apparait dans la liste de mots 
